...
import { ArrowLeft, Car, Briefcase, DollarSign } from 'lucide-react';
...
export default async function DriverSalaryDetailPage({ params }: { params: { id: string } }) {
  ...
  // 3. Fetch all salary expenses for this driver
  const driverSalaryExpenses = await db.select().from(expenses)
    .where(eq(expenses.driverId, driverId))
    .orderBy(desc(expenses.recordedAt))
    .then(list => list.filter(e => e.category === 'driver_salary'));

  // Calculate stats
  const totalCost = driverSalaryExpenses.reduce((sum, exp) => sum + exp.amountRub, 0);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" asChild className="rounded-xl bg-white shadow-sm border-slate-200">
            <Link href="/dashboard/expenses?category=driver_salary">
          ...
          <div>
            ...
            <p className="text-slate-500 mt-1 font-semibold flex items-center gap-2">
              <span>История выплат зарплаты</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border-0 shadow-sm ring-1 ring-slate-100 rounded-2xl bg-gradient-to-br from-blue-500/5 to-cyan-500/5 relative overflow-hidden">
          <div className="absolute right-0 bottom-0 translate-x-4 translate-y-4 opacity-5 pointer-events-none">
            <Briefcase className="h-32 w-32 text-blue-500" />
          </div>
        ...
        <Card className="border-0 shadow-sm ring-1 ring-slate-100 rounded-2xl bg-gradient-to-br from-rose-500/5 to-pink-500/5 relative overflow-hidden">
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-400 mt-1 font-medium">Сумма всех выплаченных зарплат</p>
          </CardContent>
        </Card>
      </div>

      {/* Full Salary Ledger Card */}
