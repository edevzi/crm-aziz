...
import { UtilizationForm } from '@/components/forms/UtilizationForm';
...
interface UtilizationExpense {
  id: number;
  category: 'utilization';
  amountRub: number;
  note: string | null;
  driverId: number | null;
  liters: number | null; // using liters for m3
  recordedAt: Date | string;
}

interface DriverUtilizationTrackerProps {
  dict: any;
  drivers: Driver[];
  expenses: UtilizationExpense[];
}

export function DriverUtilizationTracker({ dict, drivers, expenses }: DriverUtilizationTrackerProps) {
  ...
  const driverStats = drivers.map(driver => {
    ...
    const totalM3 = driverExpenses.reduce((sum, exp) => sum + (exp.liters || 0), 0);
  ...
  const unassignedM3 = unassignedExpensesList.reduce((sum, exp) => sum + (exp.liters || 0), 0);
  const totalM3All = expenses.reduce((sum, exp) => sum + (exp.liters || 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-100/80">
        ...
        <div className="relative w-full md:w-80">
          ...
          <Input
            placeholder="Поиск водителя или авто..."
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border-0 shadow-sm ring-1 ring-slate-100 rounded-2xl bg-gradient-to-br from-indigo-500/5 to-purple-500/5 relative overflow-hidden">
          <div className="absolute right-0 bottom-0 translate-x-4 translate-y-4 opacity-5 pointer-events-none">
            <Recycle className="h-40 w-40 text-indigo-500" />
          </div>
          ...
          <CardContent>
            <div className="text-3xl font-extrabold text-slate-800 flex items-baseline gap-1.5">
              {totalM3All.toLocaleString()}
              <span className="text-base font-semibold text-slate-500">м³</span>
            </div>
            <p className="text-xs text-slate-400 mt-1 font-medium">Общий объем мусора по всем водителям</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-sm ring-1 ring-slate-100 rounded-2xl bg-gradient-to-br from-rose-500/5 to-pink-500/5 relative overflow-hidden">
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-400 mt-1 font-medium">Финансовые затраты на свалку</p>
          </CardContent>
        </Card>
      </div>
                  ...
                  <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        <span className="text-xs text-slate-400 font-semibold block">Объем</span>
                        <span className="font-bold text-slate-800">{stats.totalM3.toLocaleString()} м³</span>
                      </div>
                      <div className="text-right flex flex-col items-end">
                        ...
                        <span className="inline-flex items-center text-[9px] font-extrabold text-slate-500 bg-slate-100 border border-slate-200/80 px-1.5 py-0.5 rounded mt-1 shadow-sm">
                          {totalCostAll > 0 ? Math.round((stats.totalCost / totalCostAll) * 100) : 0}% от свалки
                        </span>
                      </div>
                ...
                {isExpanded && (
                  <div className="border-t border-slate-100 bg-slate-50/40 p-4 sm:p-6 space-y-4">
                    <div className="flex justify-between items-center">
                      <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">История свалки водителем</h4>
                    ...
                    <div className="bg-white rounded-xl overflow-x-auto ring-1 ring-slate-100 shadow-inner">
                          ...
                          {stats.expensesList.slice(0, 5).map(exp => (
                            <TableRow key={exp.id} className="hover:bg-slate-50/50">
                              ...
                              <TableCell className="text-right">
                                {/* Using ExpenseForm might not match UtilizationLogs directly. But since we store in expenses, we can just use nothing or a specialized form here if needed. Since we only want to view, or we can use empty cell for now. */}
                              </TableCell>
                            </TableRow>
                          ))}
                          {stats.expensesList.length === 0 && (
                            <TableRow>
                              <TableCell colSpan={6} className="text-center py-6 text-slate-400 font-medium">
                                Нет записей о свалке для этого водителя.
                              </TableCell>
                            </TableRow>
                          )}
                        </TableBody>
                      ...
                      {stats.expensesList.length > 5 && (
                        <div className="flex justify-center p-3.5 bg-slate-50/50 border-t border-slate-100">
                          <Link href={`/drivers/${driver.id}/utilization`} className="w-full sm:w-auto">
                            <Button
                              variant="ghost"
                              size="sm"
          ...
          {unassignedExpensesList.length > 0 && (
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <span className="text-xs text-slate-400 font-semibold block">Объем</span>
                      <span className="font-bold text-slate-800">{unassignedM3.toLocaleString()} м³</span>
                    </div>
                    <div className="text-right flex flex-col items-end">
                      <span className="text-xs text-slate-400 font-semibold block">Сумма</span>
                      <span className="font-extrabold text-slate-700">{unassignedCost.toLocaleString()} RUB</span>
                      <span className="inline-flex items-center text-[9px] font-extrabold text-slate-500 bg-slate-100 border border-slate-200/80 px-1.5 py-0.5 rounded mt-1 shadow-sm">
                        {totalCostAll > 0 ? Math.round((unassignedCost / totalCostAll) * 100) : 0}% от свалки
                      </span>
                    </div>
                    <div className="text-right hidden md:block">
                      <span className="text-xs text-slate-400 font-semibold block">Записи</span>
              ...
              {expandedDriverId === -1 && (
                <div className="border-t border-slate-100 bg-slate-50/40 p-4 sm:p-6 space-y-4">
                  <div className="flex justify-between items-center">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">История нераспределенной свалки</h4>
            ...
            <Table>
              <TableHeader className="bg-slate-50/80">
                <TableRow>
                  ...
                  <SortableTableHead sortKey="liters" currentSortKey={sortKey as string} currentSortDirection={sortDirection} onSort={toggleSort} className="text-right w-[100px]">Объем</SortableTableHead>
                ...
                {sortedAllExpenses.map((expense) => {
                  ...
                  return (
                    <TableRow key={expense.id} className="hover:bg-slate-50/30">
                      ...
                      <TableCell className="text-right text-xs font-bold text-slate-800">
                        {expense.liters ? `${expense.liters} м³` : '-'}
                      </TableCell>
                      <TableCell className="text-right text-sm text-red-600 font-extrabold">
